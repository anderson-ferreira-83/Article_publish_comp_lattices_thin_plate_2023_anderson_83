function func3_lattice2_rectangular_plot_line_fig2 = rectangular_dispersion(i_fi_f,i_ff_f)
%%
i_fi = i_fi_f;
i_ff = i_ff_f;
...
data_pwe_res_freq=csvread('2_0_a_0.1_h_0.002_lattice_rectangular_all_data_res.csv');
% Load Data PWE Bare Plate
w_kk_pwe_bare = csvread('2_1_a_0.10_h_0.002_lattice_r_bare_pwe_w_data_full_path.csv');
freq_pwe_bare = csvread('2_2_a_0.10_h_0.002_lattice_r_bare_pwe_f_data_full_path.csv');
%----------------------------------------------------------------------
%%
...
% for sub_plot_freq=[10:10:100,99,110:10:190]
%     Load Data PWE Resonator
%     Numberwave
%     pwe_file_w_res = strcat('2_1_a_0.10_h_0.002_lattice_r_res_',...
%         num2str(sub_plot_freq),'_pwe_w_data_full_path.csv');
%     w_kk_pwe_res = csvread(pwe_file_w_res);
%     Frequency
%     pwe_file_f_res = strcat('2_2_a_0.10_h_0.002_lattice_r_res_',...
...
%     completos
%     if (delta_f_gap >= 1)
%         rectangle_position = [0, freq_1(index10), 3,delta_f_gap]; % [x, y, largura, altura]
...
%     end
%     legendText{cont} = sprintf('K-L: Rectangular Lattice (PWE): %g [Hz]',sub_plot_freq);
%     if (delta_f_gap >= 1)
%         sz1 = 40;
%         scatter(w_kk_1(index10),freq_1(index10),sz1,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
%         scatter(w_kk_2(index20),freq_2(index20),sz1,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
%     end
%     set(gca,'YScale', 'log')
...
%     xlabel({'M \rightarrow \Gamma \rightarrow X \rightarrow M'},'fontsize',20,'fontweight','b')
%     han.YLabel.Visible='on';
%     ylabel(han,'Frequency [Hz]','fontsize',22,'fontweight','b');
%     xlabel(han,'yourXLabel');
%     title(han,'Rectangular reduced wave vector k','fontsize',22,'fontweight','b');
%     cont_sub_plot = cont_sub_plot +1;
% end
% Definindo o rotulo da figura
% label_file_res = sprintf('2_3_a_%g_h_%g_lattice_rectangular_res',0.1,0.002);
...
% scatter(data_res(:,1),data_res(:,2),100,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1.2)
% set(gca,'fontSize',8,'fontweight','b')
...
% scatter(data_res(:,1),data_res(:,3),100,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1.2)
% set(gca,'fontSize',8,'fontweight','b')
...
% plot(data_pwe_res_freq(:,3),data_pwe_res_freq(:,6),'-b','LineWidth',4)
% scatter([10:10:100,99,110:10:190],data_res(:,4),100,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1.2)
% scatter(10:10:200,data_res(:,4),70,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1.5)
% legend(legendText)
...
% grid on
% title('Full band gap witdth in Rectangular lattice','fontsize',15)
% set(gca,'fontSize',15,'fontweight','b')
% ylim([0 23])
% 
% Give common xlabel, ylabel and title to your figure
...
% ylabel(han,'Frequency [Hz]','fontsize',22,'fontweight','b');
% title(han,'Full band gap in rectangular lattice','fontsize',22,'fontweight','b');
% Definindo o rotulo da figura
% label_file_res = sprintf('2_4_a_%g_h_%g_lattice_rectangular_res',0.1,0.002);
% Obtendo o caminho do diretorio acima do diretorio atual
% [pai, ~, ~] = fileparts(pwd);
% Exportando figura com extensao .fig
...
% csvFileName = sprintf('2_5_a_%g_h_%g_lattice_rectangular_data_res.csv',0.1,0.002);
...
% latexFileName1 = '2_0_rectangular_lattice_data_res_full.tex';
% latexFileName2 = '2_1_rectangular_lattice_data_res.tex';
% latexFileName3 = '2_2_rectangular_lattice_data_res.tex';
% latexFileName4 = '2_3_rectangular_lattice_data_res.tex';
% Obtendo o caminho do diretorio acima do diretorio atual
% [pai, ~, ~] = fileparts(pwd);
...
% 
% fprintf(latexFile1, '\\caption{Frequencies of the lower edge in mode 1, upper edge in mode 2, and the width of the full band gap associated with these modes for rectangular lattice.}\n');
% fprintf(latexFile1, '\\label{tab200}\n');
% 
...
for sub_plot_freq=[10,99,150]
    ...
    %legendText{cont} = sprintf('K-L: Rectangular Lattice (PWE): %g [Hz]',sub_plot_freq);
    %if (delta_f_gap >= 1) 
        scatter(w_kk_1(index10),freq_1(index10),70,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
        scatter(w_kk_2(index20),freq_2(index20),70,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
    %end
    ...
    sprintf('K-L: Mode %g - LRSC in rectangular lattice (PWE)',1),...
    sprintf('K-L: Mode %g - LRSC in rectangular lattice (PWE)',2),...
    sprintf('K-L: Mode %g - LRSC in rectangular lattice (PWE)',3),...
    sprintf('K-L: Bare Plate - LRSC in rectangular lattice (PWE)'),...
...
% Definindo o rotulo da figura
label_file_res = sprintf('2_6_a_%g_h_%g_lattice_rectangular_res',0.1,0.002);
% Obtendo o caminho do diretorio acima do diretorio atual
[pai, ~, ~] = fileparts(pwd);
% Exportando figura com extensao .fig
