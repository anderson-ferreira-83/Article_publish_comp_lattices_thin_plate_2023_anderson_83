function func3_lattice5_kagome_plot_line_fig2 = kagome_dispersion(i_fi_f,i_ff_f)
%%
i_fi = i_fi_f;
i_ff = i_ff_f;
...
data_pwe_res_freq=csvread('5_0_a_0.1_h_0.002_lattice_kagome_all_data_res.csv');
% Load Data PWE Bare Plate
w_kk_pwe_bare = csvread('5_1_a_0.10_h_0.002_lattice_k_bare_pwe_w_data_full_path.csv');
freq_pwe_bare = csvread('5_2_a_0.10_h_0.002_lattice_k_bare_pwe_f_data_full_path.csv');
%----------------------------------------------------------------------
% %%
...
%     % Numberwave
%     pwe_file_w_res = strcat('5_1_a_0.10_h_0.002_lattice_k_res_',...
%         num2str(sub_plot_freq),'_pwe_w_data_full_path.csv');
%     w_kk_pwe_res = csvread(pwe_file_w_res);
%     % Frequency
%     pwe_file_f_res = strcat('5_2_a_0.10_h_0.002_lattice_k_res_',...
...
%         rectangle('Position', rectangle_position, 'FaceColor', 'y', 'EdgeColor', 'none')
%     end
%     % Modos 2 e 3
%     if (delta_f_gap1 >1)
...
%     % Modos 4 e 5
%     if (delta_f_gap3 >1)
%         rectangle_position = [0, freq_4(index13), 5,delta_f_gap3]; % [x, y, largura, altura]
...
%     end
%     %legendText{cont} = sprintf('K-L: Kagome Lattice (PWE): %g [Hz]',sub_plot_freq);
%     if (delta_f_gap0 > 1)
%         sz1 = 40;
%         scatter(w_kk_1(index10),freq_1(index10),sz1,'MarkerEdgeColor','k','MarkerFaceColor','r','LineWidth',1)
%         scatter(w_kk_2(index20),freq_2(index20),sz1,'MarkerEdgeColor','k','MarkerFaceColor','r','LineWidth',1)
%     end
%     if (delta_f_gap1 > 1)
%         sz1 = 40;
...
%     if (delta_f_gap2 > 1)
%         sz1 = 40;
%         scatter(w_kk_3(index12),freq_3(index12),sz1,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1)
%         scatter(w_kk_4(index22),freq_4(index22),sz1,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1)
%     end
%     if (delta_f_gap3 > 1)
%         sz1 = 40;
%         scatter(w_kk_4(index13),freq_4(index13),sz1,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
%         scatter(w_kk_5(index23),freq_5(index23),sz1,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
%     end
%     set(gca,'YScale', 'log')
...
%         'DisplayName',sprintf('fr =  %g [Hz],\\Deltaf_{34} = %.1f [Hz],\\Deltaf_{45} = %.1f [Hz]',...
%         sub_plot_freq,delta_f_gap2,delta_f_gap3),...
...
%     title(han,'Kagome reduced wave vector k','fontsize',22,'fontweight','b');
%     cont_sub_plot = cont_sub_plot +1;
% end
% % Definindo o rotulo da figura
% label_file_res = sprintf('5_3_a_%g_h_%g_lattice_kagome_hexagonal_res',0.1,0.002);
...
% legendText{1} = sprintf('Max \\Deltaf_{34} = %.2f [Hz] in fr = %d [Hz]',...
%     data_pwe_res_freq(indice_maximo1,12), data_pwe_res_freq(indice_maximo1,3));
% legendText{2} = ('Curve Fbgw beteween Modes 3 and 4');
% % Modos 4 e 5
% % Encontre o valor máximo da curva e sua posição
% [maximo2, indice_maximo2] = max(data_pwe_res_freq(:,15));
% legendText{3} = sprintf('Max \\Deltaf_{45} = %.2f [Hz] in fr = %d [Hz]',...
%     data_pwe_res_freq(indice_maximo2,15), data_pwe_res_freq(indice_maximo2,3));
% legendText{4} = ('Curve FBGW beteween Modes 4 and 5');
% % Figura 2
% fig2 = figure(12);
% subplot(1,3,1)
% box on
...
%     120,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1.5)
% plot(data_pwe_res_freq(:,3),data_pwe_res_freq(:,12),'-g','LineWidth',4)
% scatter(10:10:200,data_res(:,4),100,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1.5)
...
%     120,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',2)
% plot(data_pwe_res_freq(:,3),data_pwe_res_freq(:,15),'-b','LineWidth',4)
...
% % scatter(data_pwe_res_freq(indice_maximo1,3),data_pwe_res_freq(indice_maximo1,12),...
% %     120,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1.5)
% % plot(data_pwe_res_freq(:,3),data_pwe_res_freq(:,12),'-g','LineWidth',3)
% % scatter(data_pwe_res_freq(indice_maximo2,3),data_pwe_res_freq(indice_maximo2,15),...
% %     100,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1.5)
% % plot(data_pwe_res_freq(:,3),data_pwe_res_freq(:,15),'-b','LineWidth',3)
% % scatter(10:10:200,data_res(:,4),100,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1.5)
% % scatter(10:10:200,data_res(:,7),100,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1.5)
% % %legend(legendText)
% % %set(legend,'FontSize',12);
...
% %
% % Definindo o rotulo da figura
% label_file_res = sprintf('5_4_a_%g_h_%g_lattice_kagome_res',0.1,0.002);
% % Obtendo o caminho do diretorio acima do diretorio atual
% [pai, ~, ~] = fileparts(pwd);
% % Exportando figura com extensao .fig
...
% csvFileName = sprintf('5_5_a_%g_h_%g_lattice_kagome_data_res.csv',0.1,0.002);
...
% latexFileName1 = '5_0_kagome_lattice_data_res_full.tex';
% latexFileName2 = '5_1_kagome_lattice_data_res.tex';
% latexFileName3 = '5_2_kagome_lattice_data_res.tex';
% latexFileName4 = '5_3_kagome_lattice_data_res.tex';
% latexFileName5 = '5_4_kagome_lattice_data_res.tex';
% 
% % Obtendo o caminho do diretorio acima do diretorio atual
% [pai, ~, ~] = fileparts(pwd);
...
% %
% fprintf(latexFile1, '\\caption{The lower and upper edge frequencies in modes 3 and 4, along with the width of the first full band gap; The lower and upper edge frequencies in modes 4 and 5, alongside the width of the second full band gap in a Kagome lattice.}\n');
% fprintf(latexFile1, '\\label{tab500}\n');
% %
...
% %
% fprintf(latexFile1, '$f_R$[Hz] & $f_3$[Hz] & $f_4$[Hz] & $\\Delta f _{34}$[Hz] & $f_4$[Hz] & $f_5$[Hz] & $\\Delta f _{45}$[Hz]\\\\ \\hline\n');
% fprintf(latexFile2, '$f_R$[Hz] & $f_3$[Hz] & $f_4$[Hz]  \\\\ \\hline\n');
% fprintf(latexFile3, '$f_R$[Hz] & $f_4$[Hz] & $f_5$[Hz]  \\\\ \\hline\n');
% fprintf(latexFile4, '$f_R$[Hz] & $\\Delta f_{34}$[Hz]  \\\\ \\hline\n');
% fprintf(latexFile5, '$f_R$[Hz] & $\\Delta f_{45}$[Hz]  \\\\ \\hline\n');
...
for sub_plot_freq=[10,50,150]
    ...
    for i=1:7
        plot_pwe_res{i} = plot(w_kk_pwe_res(i,:),freq_pwe_res(i,:),'Color',colorPalette{i},'LineWidth',4)
    end
    for i=1:7
        plot_pwe_bare{i} = plot(w_kk_pwe_bare(i,:),freq_pwe_bare(i,:),...
            'DisplayName',sprintf('Rectangular Lattice - Bare Plate'),...
            'LineStyle', '-','LineWidth', 2,'Color','k');
    end
    %legendText{cont} = sprintf('K-L: Kagome Lattice (PWE): %g [Hz]',sub_plot_freq);
    if (delta_f_gap0 > 1)
        scatter(w_kk_1(index10),freq_1(index10),70,'MarkerEdgeColor','k','MarkerFaceColor','r','LineWidth',1)
        scatter(w_kk_2(index20),freq_2(index20),70,'MarkerEdgeColor','k','MarkerFaceColor','r','LineWidth',1)
    end
    if (delta_f_gap1 > 1)
    ...
    if (delta_f_gap2 > 1)
        scatter(w_kk_3(index12),freq_3(index12),70,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1)
        scatter(w_kk_4(index22),freq_4(index22),70,'MarkerEdgeColor','k','MarkerFaceColor','g','LineWidth',1)
    end
    if (delta_f_gap3 > 1)
        scatter(w_kk_4(index13),freq_4(index13),70,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
        scatter(w_kk_5(index23),freq_5(index23),70,'MarkerEdgeColor','r','MarkerFaceColor','b','LineWidth',1)
    end
    ...
    plot_pwe_bare{1},...
    plot_res], ...
    sprintf('FBGW 1 - f_3 = %.2f [Hz], f_4 = %.2f [Hz], \\Deltaf_{34} = %.2f [Hz]',...
    freq_3(index12),freq_4(index22),delta_f_gap2),...
    sprintf('FBGW 2 - f_4 = %.2f [Hz], f_5 = %.2f [Hz], \\Deltaf_{45} = %.2f [Hz]',...
    freq_4(index13),freq_5(index23),delta_f_gap3),...
    sprintf('K-L: Mode %g - LRSC in kagome lattice (PWE)',3),...
    sprintf('K-L: Mode %g - LRSC in kagome lattice (PWE)',4),...
    sprintf('K-L: Mode %g - LRSC in kagome lattice (PWE)',5),...
    sprintf('K-L: Bare Plate - LRSC in kagome lattice (PWE)'),...
...
%title('Kagome reduced wave vector k','fontsize',22,'fontweight','b');
% Definindo o rotulo da figura
label_file_res = sprintf('5_6_a_%g_h_%g_lattice_kagome_res',0.1,0.002);
% Obtendo o caminho do diretorio acima do diretorio atual
[pai, ~, ~] = fileparts(pwd);
% Exportando figura com extensao .fig
