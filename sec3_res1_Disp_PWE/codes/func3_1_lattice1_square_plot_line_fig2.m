function func3_1_lattice1_square_plot_line_fig2 = square_dispersion(i_fi_f,i_ff_f)
%%
i_fi = i_fi_f;
i_ff = i_ff_f;
...
data_pwe_res_freq=csvread('1_0_a_0.1_h_0.002_lattice_square_all_data_res.csv');
% Load Data PWE Bare Plate
w_kk_pwe_bare = csvread('1_1_a_0.10_h_0.002_lattice_s_bare_pwe_w_data_full_path.csv');
freq_pwe_bare = csvread('1_2_a_0.10_h_0.002_lattice_s_bare_pwe_f_data_full_path.csv');
%----------------------------------------------------------------------
%%
...
% for sub_plot_freq=[10:10:100,105,110:10:190]
%     % Load Data PWE Resonator
%     % Numberwave
%     pwe_file_w_res = strcat('1_1_a_0.10_h_0.002_lattice_s_res_',...
%         num2str(sub_plot_freq),'_pwe_w_data_full_path.csv');
%     w_kk_pwe_res = csvread(pwe_file_w_res);
%     % Frequency
%     pwe_file_f_res = strcat('1_2_a_0.10_h_0.002_lattice_s_res_',...
...
%         rectangle('Position', rectangle_position, 'FaceColor', 'm', 'EdgeColor', 'none')
%     end
%     for i=1:3
%         plot(w_kk_pwe_res(i,:),freq_pwe_res(i,:),'Color',colorPalette{i},'LineWidth',2)
%         plot(w_kk_pwe_bare(i,:),freq_pwe_bare(i,:),'--k','LineWidth',1)
%     end
%     %legendText{cont} = sprintf('K-L: Square Lattice (PWE): %g [Hz]',sub_plot_freq);
%     if (delta_f_gap > 1)
%         sz1 = 40;
%         scatter(w_kk_1(index10),freq_1(index10),sz1,'MarkerEdgeColor','k','MarkerFaceColor','m','LineWidth',1)
%         scatter(w_kk_2(index20),freq_2(index20),sz1,'MarkerEdgeColor','k','MarkerFaceColor','m','LineWidth',1)
%     end
%     set(gca,'YScale', 'log')
...
%     xlabel(han,{'M \rightarrow \Gamma \rightarrow X \rightarrow M'},'fontsize',22,'fontweight','b');
%     ylabel(han,'Frequency [Hz]','fontsize',22,'fontweight','b');
%     %title(han,'Square reduced wave vector k','fontsize',22,'fontweight','b');
%     cont_sub_plot = cont_sub_plot +1;
% end
% % Definindo o rotulo da figura
% label_file_res = sprintf('1_3_a_%g_h_%g_lattice_square_res',0.1,0.002);
...
% %%
% % Criando e exportando dados das 15 sim da Rede quadrada
% % Specify the CSV file name
% csvFileName = sprintf('1_5_a_%g_h_%g_lattice_square_data_res.csv',0.1,0.002);
...
% latexFileName1 = '1_0_square_lattice_data_res_full.tex';
% latexFileName2 = '1_1_square_lattice_data_res.tex';
% latexFileName3 = '1_2_square_lattice_data_res.tex';
% latexFileName4 = '1_3_square_lattice_data_res.tex';
% % Obtendo o caminho do diretorio acima do diretorio atual
% [pai, ~, ~] = fileparts(pwd);
...
% % Abrindo/criando o arquivo do Latex para escrita
...
% %
% fprintf(latexFile1, '\\caption{Frequencies of the lower edge in mode 1, upper edge in mode 2, and the width of the full band gap associated with these modes for square lattice.}\n');
% fprintf(latexFile1, '\\label{tab100}\n');
% %
...
for sub_plot_freq=[10,105,150]
    ...
    for i=1:7
        ...
        'DisplayName',sprintf('Square Lattice - Bare Plate'),...
        'LineStyle', '-','LineWidth', 2,'Color','k');   
    end
    if (delta_f_gap > 1)
        scatter(w_kk_1(index10),freq_1(index10),120,'MarkerEdgeColor','k','MarkerFaceColor','m','LineWidth',1)
        scatter(w_kk_2(index20),freq_2(index20),120,'MarkerEdgeColor','k','MarkerFaceColor','m','LineWidth',1)
    end
...
%width = 29.7 + 5;  % Largura do papel A4 em centímetros
%height = 21 ;   % Altura do papel A4 em centímetros
%set(gcf, 'Position', [0,0, width, height]);
%title('Square reduced wave vector k','fontsize',22,'fontweight','b');
% Definindo o rotulo da figura
label_file_res = sprintf('1_6_a_%g_h_%g_lattice_square_res',0.1,0.002);
% Obtendo o caminho do diretorio acima do diretorio atual
[pai, ~, ~] = fileparts(pwd);
% Exportando figura com extensao .fig
